...
@WebServlet(name = "AjoutPanierSandwichPersoServlet", urlPatterns = {"/ajoutpaniersandwichperso"})
public class AjoutPanierSandwichPersoServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        // Permet de récupérer l'url de la page qui a appelé la servlet AjoutPanierSandwichPersoServlet
        ...
        if (request.getParameter("reset") != null){
        ...
        }else if (request.getParameter("addSandwich") != null){
            ...
            try {
                ...
                // Le produit est un sandwich personnalisé
                Produit p = daoProduct.findProductByLibelle("Sandwich Personnalisé");
                ...
                if (sessionPanier.getAttribute("panier") != null){
                    ...
                    // On récupère la liste des ingrédients que l'utilisateur veut ajouter à son sandwich
                    ...
                    if (mylistIngredientsProduct != null)
                    {
                        ...
                        {
                            // On calcule le prix total du sandwich personnalisé en ajoutant le prix de chaque ingrédient multiplié par la quantité
    ...
    private Collection<IngredientOfProduct> checkListOfCheckbox(HttpServletRequest request, HttpServletResponse response, Collection<IngredientOfProduct> mylistIngredientsProduct, Panier myPanier)
    {
        ...
        try {
            ...
            // Vérification des checkbox opt_pain
            String[] optPain = request.getParameterValues("opt_pain");

            if(optPain == null)
            {
                request.setAttribute("error_checkbox_pain", "Veuillez choisir le type de pain pour votre sandwich !");
                RequestDispatcher rd = request.getRequestDispatcher(pageAppelante);
                rd.forward(request, response);
                return null;
            }
            ...
            {
                // Si plus d'une checkbox opt_pain est cochée alors on renvoie une erreur
                if (optPain.length > 1){
                    request.setAttribute("error_checkbox_pain", "Veuillez ne saisir qu'un seul type de pain pour votre sandwich !");
                    RequestDispatcher rd = request.getRequestDispatcher(pageAppelante);
                    rd.forward(request, response);
                    return null;
                }
                ...
                String optPainValue = "";
                for (String optPainChecked : optPain) {
                    optPainValue = optPainChecked.replace("opt_pain_", "");
                    long idIngr = Long.parseLong(optPainValue);
                    i = daoIngredient.findIngredientById(idIngr);
                    // On vérifie à ce que le produit soit disponible dans la quantité demandée
                    if (myPanier == null){
                        if ((i.getStock() - 1) < 0)
                        {
                            request.setAttribute("error_checkbox_pain", "La quantité demandée : " + 1 + " de " + i.getLibelle().toString() + " n'est pas disponible en stock !");
                            RequestDispatcher rd = request.getRequestDispatcher(pageAppelante);
                            rd.forward(request, response);
                            return null;
                        }   
                                ...
                                for (IngredientOfProduct ingr : prod.getListIngredientOfProduct())
                                {
                                    ...
                                    if (ingr.getIngredient().getLibelle().equals(i.getLibelle())){
                                        ...
                                        if ((i.getStock() - (ingr.getQuantiteIngredient() + 1)) < 0){
                                            request.setAttribute("error_checkbox_pain", "La quantité demandée : 1 de " + i.getLibelle() + "<br />" +
                ...
                // A commenter
                // request.setAttribute("error_checkbox_pain", optPainValue);       
                //
            }
            // Si l'utilisateur n'a pas effectué de choix pour l'élément principal du sandwich, on renvoie un message d'erreur
            if (request.getParameter("main_element") == null)
            {
                request.setAttribute("error_checkbox_main_elem", "Veuillez choisir l'élément principal pour votre sandwich !");
                RequestDispatcher rd = request.getRequestDispatcher(pageAppelante);
                rd.forward(request, response);
                return null;
            }else{
                ...
                for (String mainElementChosen : mainElement){
                    if (mainElementChosen.equals("main_element_poisson")){
                        ...
                        if(optPoisson == null){
                            request.setAttribute("error_checkbox_main_elem", "Veuillez choisir le poisson pour votre sandwich !");
                            RequestDispatcher rd = request.getRequestDispatcher(pageAppelante);
                            rd.forward(request, response);
                            return null;
                        }else{
                            ...
                            if (optPoisson.length > 2){
                                request.setAttribute("error_checkbox_main_elem", "Veuillez ne saisir qu'un ou deux types de poisson pour votre sandwich !");
                                RequestDispatcher rd = request.getRequestDispatcher(pageAppelante);
                                rd.forward(request, response);
                                return null;
                            }
                    ...
                    if (mainElementChosen.equals("main_element_charcuterie")){
                        ...
                        if(optCharcu == null){
                            request.setAttribute("error_checkbox_main_elem", "Veuillez choisir la charcuterie pour votre sandwich !");
                            RequestDispatcher rd = request.getRequestDispatcher(pageAppelante);
                            rd.forward(request, response);
                            return null;
                        }else{
                            ...
                            if (optCharcu.length > 2){
                                request.setAttribute("error_checkbox_main_elem", "Veuillez ne saisir qu'un ou deux types de charcuterie pour votre sandwich !");
                                RequestDispatcher rd = request.getRequestDispatcher(pageAppelante);
                                rd.forward(request, response);
                                return null;
                            }
            ...
            if(optLegume != null){                        
                ...
                if (optLegume.length > 2){
                    request.setAttribute("error_checkbox_legume", "Veuillez ne saisir qu'un ou deux types de légume pour votre sandwich !");
                    RequestDispatcher rd = request.getRequestDispatcher(pageAppelante);
                    rd.forward(request, response);
                    return null;
                }
            ...
            if(optSauce != null){                        
                ...
                if (optSauce.length > 2){
                    request.setAttribute("error_checkbox_sauce", "Veuillez ne saisir qu'un ou deux types de sauce pour votre sandwich !");
                    RequestDispatcher rd = request.getRequestDispatcher(pageAppelante);
                    rd.forward(request, response);
                    return null;
                }
            ...
            if(optFromage != null){                        
                ...
                if (optFromage.length > 2){
                    request.setAttribute("error_checkbox_fromage", "Veuillez ne saisir qu'un ou deux types de fromage pour votre sandwich !");
                    RequestDispatcher rd = request.getRequestDispatcher(pageAppelante);
                    rd.forward(request, response);
                    return null;
                }
